...
"""# **DrugNet Result Analysis**"""

generated_mols_general=np.empty([1,7050,84],dtype=np.float64)
count=0
for i in range(1,16):
  mols=np.load("/content/drive/MyDrive/DrugNet/Generated_mols/generated_sols_"+str(i)+".npy",allow_pickle=True)
  for j in range(np.shape(mols)[1]):
    generated_mols_general[0][count]=mols[0][j]
    count+=1

generated_mols_fitness=list()
for i in range(7050):
  generated_mols_fitness.append(-fitness_func(np.reshape(generated_mols_general[0][i],(1,84))))

filename = '/content/drive/MyDrive/DrugNet/Generated_mols/generated_mols_fitness'
joblib.dump(generated_mols_fitness, filename)

generated_mols_fitness=joblib.load('/content/drive/MyDrive/DrugNet/Generated_mols/generated_mols_fitness')

generated_mols_solution=np.empty([1,308,84],dtype=object)
index_count=0
for i in range(7050):
  if generated_mols_fitness[i]>13.13:
   generated_mols_solution[0][index_count]=generated_mols_general[0][i:i+1]
   index_count+=1

np.save("/content/drive/MyDrive/DrugNet/Generated_COVID_solutions",generated_mols_solution)

sum(generated_mols_fitness_screened)/len(generated_mols_fitness_screened)

max(generated_mols_fitness_screened)

import seaborn as sns

generated_mols_fitness_screened=list()
for i in generated_mols_fitness:
  if i>13.13:
    generated_mols_fitness_screened.append(i)

min(generated_mols_fitness_screened)

max(generated_mols_fitness_screened)

sum(generated_mols_fitness_screened)/len(generated_mols_fitness_screened)

len(generated_mols_fitness_screened)

generated_mols_fitness=generated_mols_fitness_screened

import matplotlib.pyplot as plt
import seaborn as sns

covid_mols_fitness=results

covid_mols_fitness

fitness_scores=[generated_mols_fitness_screened,covid_mols_fitness]
labels=["İlaçNet","COVID-19 Veri Seti"]
for i in range(2):
  sns.distplot(fitness_scores[i], hist=False,kde=True, 
              bins=int(60), 
              kde_kws = {'shade': True, 'linewidth':3,'bw_method':0.75},label=labels[i])
plt.legend(prop={'size': 10},loc="upper left")
plt.xlabel('Uygunluk Değeri')
plt.ylabel('Yoğunluk')

sns.distplot(generated_mols_fitness_screened, hist=False,kde=True, 
             bins=int(60), 
             kde_kws={'linewidth': 6})

covid_mols=np.load("/content/drive/MyDrive/DrugNet/COVID_SELFIES.npy",allow_pickle=True)

np.shape(covid_mols)

covid_mols_new=np.empty((6566,268))
for j in range(6566):
  for i in range(128):
    covid_mols_new[j][i]=covid_mols[j][i]
  for i in range(128,268):
    covid_mols_new[j][i]=0

np.shape(covid_mols_new)

covid_mols_fitness=list()
for i in range(np.shape(covid_mols)[0]):
  covid_mols_fitness.append(deep_adtp([covid_3cl_proteiase,covid_mols_new[i:i+1]]))

filename = '/content/drive/MyDrive/DrugNet/cov_mols_fitness'
joblib.dump(covid_mols_fitness, filename)

covid_mols_fitness=joblib.load('/content/drive/MyDrive/DrugNet/covid_mols_fitness')

covid_mols_fitness_screened=list()
for i in covid_mols_fitness:
  if i>13:
    covid_mols_fitness_screened.append(i)

len(covid_mols_fitness_screened)
